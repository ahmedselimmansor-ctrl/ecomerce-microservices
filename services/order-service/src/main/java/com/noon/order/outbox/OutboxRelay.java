...
public class OutboxRelay {
    ...
    public OutboxRelay(OutboxRepository outbox,
                       ...
                       @Value("${noon.outbox.batch-size:100}") int batchSize,
                       @Value("${noon.outbox.max-attempts:10}") int maxAttempts) {
        this.outbox = outbox;
        this.kafka = kafka;
        this.batchSize = batchSize;
        this.maxAttempts = maxAttempts;

        Gauge.builder("noon.outbox.pending", pendingGauge, AtomicLong::get)
                .description("Outbox rows awaiting publication")
                .register(metrics);
    }

    @Scheduled(fixedDelayString = "${noon.outbox.poll-delay-ms:500}")
    @Transactional
    public void relay() {
