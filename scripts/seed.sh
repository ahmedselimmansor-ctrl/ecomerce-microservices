...
#  المستخدم التجريبي كذلك (V2__seed_admin.sql): demo@topchoice.local / Passw0rd!
# ============================================================================
set -euo pipefail
...
mongosh_cmd() {
  "${COMPOSE[@]}" exec -T mongo mongosh \
    --quiet --username "${MONGO_USER:-topchoice}" --password "${MONGO_PASS:-topchoice_local_pw}" \
    --authenticationDatabase admin topchoice_catalog --eval "$1"
}
...
upsert "$(cat <<JSON
{
  "sku": "TC-APL-IP15-128-BLK", "slug": "apple-iphone-15-128gb-black",
  ...
  "tags": ["express", "bestseller"], "sellerId": "topchoice-retail", "status": "ACTIVE"
}
JSON
)"
...
upsert "$(cat <<JSON
{
  "sku": "TC-APL-IP15-256-BLU", "slug": "apple-iphone-15-256gb-blue",
  ...
  "tags": ["express"], "sellerId": "topchoice-retail", "status": "ACTIVE"
}
JSON
)"
...
upsert "$(cat <<JSON
{
  "sku": "TC-SAM-S24-256-GRY", "slug": "samsung-galaxy-s24-256gb-grey",
...
upsert "$(cat <<JSON
{
  "sku": "TC-SAM-S24U-512-TTN", "slug": "samsung-galaxy-s24-ultra-512gb",
  ...
  "tags": ["bestseller"], "sellerId": "topchoice-retail", "status": "ACTIVE"
}
JSON
)"
...
upsert "$(cat <<JSON
{
  "sku": "TC-APL-MBA-M3-256", "slug": "apple-macbook-air-m3-256gb",
...
upsert "$(cat <<JSON
{
  "sku": "TC-SON-WH1000XM5-BLK", "slug": "sony-wh-1000xm5-black",
...
upsert "$(cat <<JSON
{
  "sku": "TC-APL-AIRPODS-PRO2", "slug": "apple-airpods-pro-2",
...
upsert "$(cat <<JSON
{
  "sku": "TC-LG-OLED55C4", "slug": "lg-oled55c4-4k-tv",
  ...
  "tags": [], "sellerId": "topchoice-retail", "status": "ACTIVE"
}
JSON
)"
...
upsert "$(cat <<JSON
{
  "sku": "TC-PSN-PS5-SLIM-DE", "slug": "playstation-5-slim-digital",
...
upsert "$(cat <<JSON
{
  "sku": "TC-XBX-SERIESX-1TB", "slug": "xbox-series-x-1tb",
...
upsert "$(cat <<JSON
{
  "sku": "TC-NIK-AIRMAX270-42", "slug": "nike-air-max-270-42",
  ...
  "tags": ["express"], "sellerId": "topchoice-fashion", "status": "ACTIVE"
}
JSON
)"
...
upsert "$(cat <<JSON
{
  "sku": "TC-ADI-ULTRA22-43", "slug": "adidas-ultraboost-22-43",
...
upsert "$(cat <<JSON
{
  "sku": "TC-LOR-REV-SERUM-30", "slug": "loreal-revitalift-serum-30ml",
  ...
  "tags": ["express"], "sellerId": "topchoice-beauty", "status": "ACTIVE"
}
JSON
)"
...
upsert "$(cat <<JSON
{
  "sku": "TC-DYS-V15-DETECT", "slug": "dyson-v15-detect",
  ...
  "tags": [], "sellerId": "topchoice-home", "status": "ACTIVE"
}
JSON
)"
...
upsert "$(cat <<JSON
{
  "sku": "TC-NES-VERTUO-POP", "slug": "nespresso-vertuo-pop",
  ...
  "tags": ["express"], "sellerId": "topchoice-home", "status": "ACTIVE"
}
JSON
)"
...
upsert "$(cat <<JSON
{
  "sku": "TC-IKE-DESK-140", "slug": "office-desk-140cm",
