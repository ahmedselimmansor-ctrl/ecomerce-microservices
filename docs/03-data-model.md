...
**الترحيل المُطبَّق لا يُعدَّل.** Flyway يحفظ بصمة كل ملف طبّقه؛ تغيير حرف
واحد فيه يجعله يرفض الإقلاع بـ `Migrations have failed validation` على كل
قاعدة بيانات سبق أن طُبِّق عليها.

ما يجعل هذا الخطأ خطيرًا تحديدًا أن **بيئة جديدة تعمل بلا شكوى**: القاعدة
الفارغة تطبّق الملف المعدَّل وتحفظ بصمته الجديدة فلا تعارض. البيئات القائمة
وحدها هي التي تسقط. أي أن الاختبار على قاعدة نظيفة يمرّ، والعطل ينتظر النشر
على قاعدة حقيقية — وهو ما وقع فعلًا في هذا المستودع عند تغيير العملة
الافتراضية داخل `V1__init.sql`.

| ما تريده | الطريق الصحيح |
|---|---|
| تغيير عمود أو افتراضي | ترحيل جديد `V<n>__…` بـ `ALTER TABLE` |
| إصلاح ترحيل لم يُطبَّق بعد في أي بيئة | تعديله مباشرةً مقبول |
| تصحيح خطأ في ترحيل مُطبَّق | ترحيل جديد يصحّحه — لا تراجُع |
| بيانات عرض | ترحيل محكوم بعلامة `${seedDemoData}` |

**البيانات القائمة لا تُعاد كتابتها بأثر رجعي** إلا بقرار صريح. عملة الطلب
مثلًا لقطة وقت الشراء لا إعدادًا حاليًا: تغييرها على صفوف سابقة يزوّر فواتير
صدرت بعملة أخرى. لذلك ترحيل العملة غيّر الافتراضي للصفوف الجديدة فقط.
