...
| **الاختبارات** | 256 اختبار وحدة · 9 اختبارات تزاحم على PostgreSQL حقيقي · 40 حالة على المتجر · 28 على لوحة التحكم |
...
| `api-gateway` | حجب المسارات الإدارية عند الحافة: 40 حالة ترميز مختلفة لنفس الكلمة (مئوي، مزدوج، معاملات مصفوفة، تجاوز مسار) بعد ثغرة سمحت بوصول إداري كامل بلا مصادقة |
| `cart-service` | الدمج عند تسجيل الدخول يأخذ الأكبر لا المجموع — الجمع يعطي العميل ضعف ما أراد حين يضيف نفس المنتج على جهازين |
| `notification-service` | الهروب من HTML في كل حقل يتحكم فيه المستخدم أو شركة الشحن: القالب لا يمرّ بأي تنظيف آخر قبل الإرسال |
| `search-service` | ترقيم الصفحات يقرّب لأعلى (21 نتيجة = صفحتان)، ونطاقات السعر متلاصقة بلا فجوات، والتجميع على الحقل المُعرَّف في الخريطة |
| `recommendation-service` | المحرك الاحتياطي: استبعاد ما شُوهد، بلا تكرار، وسقوط Redis يُفرغ القسم ولا يُسقط الصفحة |
| `frontend` | القسمة على 100 تحدث عند العرض فقط، وكاش المُنسِّقات لا يخلط بين نسخة بعملة وأخرى بدونها |

### اختبارات التكامل

اختبارات الوحدة تفحص المنطق في الذاكرة؛ بعض أخطر السلوك لا يعيش هناك أصلًا.
`make integration` يشغّل تسعة اختبارات على PostgreSQL حقيقي:

**البيع الزائد.** ثلاثون خيطًا تتسابق على عشرين قطعة بانطلاقة واحدة. منطق
`StockItem.reserve` صحيح تمامًا في الذاكرة، لكن طلبين متزامنين يقرآن الصف نفسه
قبل أن يكتب أحدهما فيرى كلاهما «متاح» ويحجزان معًا. ما يمنع ذلك هو
`SELECT … FOR UPDATE` على مستوى المحرّك — سلوك لا وجود له في قاعدة داخل الذاكرة.

**قيود المخطّط.** `CHECK (on_hand >= 0)` تعيش في قاعدة البيانات لا في الكود،
وهي الشبكة الأخيرة لو تسرّب مسار كتابة يلتفّ على منطق المجال.

**الـ Outbox والقفل المتفائل.** ثماني «نسخ» تستقصي الطابور معًا فلا يُلتقط صف
مرتين، والكتابة الضائعة على الطلب تُرفض بـ optimistic lock بدل أن تمحو تأكيد
دفع وصل في اللحظة نفسها.

> **تحقّقنا أن الاختبارات ليست خضراء فارغة** بإزالة الآلية المفحوصة والتأكد من
> الفشل. وهذا كشف عيبًا حقيقيًا في اختبار كتبناه: «لا تكرار في الـ outbox» كان
> يمرّ حتى بعد إزالة `SKIP LOCKED`، لأن `FOR UPDATE` وحده يمنع التكرار أيضًا —
> يحجب المستقصي الثاني حتى يُودِع الأول. ما يميّز `SKIP LOCKED` هو **عدم
> الحجب** لا السلامة. أُعيد كتابة الاختبار: معاملة تمسك قفلًا مفتوحًا وأخرى
> تستقصي، وتفشل بـ Timeout عند إزالة `SKIP LOCKED`.

الاسم ينتهي بـ `IT` فلا يلتقطه `mvn test`؛ يشغّله failsafe في `mvn verify`
وحده. اختبار وحدة يفشل على جهاز لم يشغّل الاستاك تُتجاهَل نتيجته سريعًا،
وتلك أسرع طريقة لموت مجموعة اختبارات.
