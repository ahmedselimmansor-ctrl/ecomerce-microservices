...
export function SectionHeader({
  ...
  return (
    <header className="mb-3 flex items-center justify-between gap-4">
      <h2 className="text-xl font-extrabold text-tc-ink sm:text-2xl">{title}</h2>
      {href && (
        <Link
          href={href}
          className="shrink-0 rounded border border-tc-ink/20 px-4 py-2 text-[13px]
    ...
    <Carousel>
      {QUICK_LINKS.map((link) => (
        ...
        >
          ...
          <span className="text-center text-[13px] font-semibold leading-tight text-tc-ink">
            {link.label}
          </span>
        </Link>
      ))}
...
export function TripleRow() {
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {/* -------------------------------------------------- more reasons */}
      <section>
        <h2 className="mb-3 text-xl font-extrabold text-tc-ink">More reasons to shop</h2>
          ...
          {REASONS.map((reason) => (
            <Link
              ...
              className={`flex min-h-[132px] flex-col justify-end rounded-lg p-4 transition
                          hover:shadow-[var(--shadow-tc-hover)] ${reason.bg}`}
            >
              <span className="text-[15px] font-extrabold text-tc-ink">{reason.title}</span>
              <span className="text-[13px] text-tc-ink/70">{reason.subtitle}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* ----------------------------------------------------- mega deals */}
      <section className="rounded-lg bg-tc-plum-soft p-4">
        <h2 className="mb-3 text-xl font-extrabold text-tc-ink">Mega deals</h2>
        <div className="grid grid-cols-2 gap-3">
          {MEGA_DEALS.map((deal) => (
            <Link
              key={deal.title}
              ...
              className="relative flex min-h-[132px] flex-col justify-end rounded-lg bg-white p-3
                         transition hover:shadow-[var(--shadow-tc-hover)]"
            >
              <span className="absolute end-0 top-0 rounded-bl-lg rounded-tr-lg bg-tc-plum
                               px-2 py-1 text-[10px] font-bold text-white">
                {deal.label}
              </span>
              <span className="text-[12px] text-tc-ink/70">{deal.title}</span>
              <span className="text-[15px] font-extrabold text-tc-ink">{deal.offer}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* -------------------------------------------------------- in focus */}
      <section>
        <h2 className="mb-3 text-xl font-extrabold text-tc-ink">In focus</h2>
  ...
  return (
    <section className={banner ? 'overflow-hidden rounded-lg' : ''}>
      {banner && (
        ...
        >
          <span>
            <span className="block text-lg font-extrabold text-tc-ink">{banner.subtitle}</span>
            <span className="block text-3xl font-extrabold text-tc-berry sm:text-4xl">
              {banner.title}
            </span>
          </span>
          <span className="rounded-md bg-tc-ink px-8 py-3 text-sm font-extrabold text-white">
            {banner.cta}
          </span>
        </Link>
      )}
      ...
      <Carousel>
        {tiles.map((tile) => (
          ...
          >
            ...
            <span className="text-center text-[15px] font-bold leading-tight text-tc-ink">
              {tile.label}
            </span>
          </Link>
        ))}
...
const COUPONS = [
  ...
  { code: 'TOPCHOICE10', title: 'Everything', offer: 'GET 10% OFF', note: 'On your first order', href: '/' },
];
  ...
  return (
    <section>
      <h2 className="mb-4 text-center text-2xl font-extrabold tracking-tight">
        <span className="text-tc-amber">COUPON</span>{' '}
        <span className="text-tc-ink">ZONE</span>
      </h2>
        ...
        {COUPONS.map((coupon) => (
          <Link
            ...
            className="relative w-[260px] shrink-0 overflow-hidden rounded-lg border
                       border-tc-line bg-white p-4 pt-8 transition
                       hover:shadow-[var(--shadow-tc-hover)]"
          >
            <span className="absolute start-0 top-2 rounded-e-full bg-tc-berry px-3 py-1
                             text-[12px] font-bold text-white">
              Use code: <span className="font-extrabold">{coupon.code}</span>
            </span>
            <span className="mt-3 block text-[15px] font-semibold text-tc-ink">
              {coupon.title}
            </span>
            <span className="mt-2 block text-2xl font-extrabold leading-tight text-tc-berry">
              {coupon.offer}
            </span>
            {coupon.note && (
              <span className="mt-1 block text-[13px] text-tc-muted">{coupon.note}</span>
            )}
          </Link>
        ))}
      </Carousel>
...
export function OfficialStores() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-extrabold tracking-tight text-tc-ink">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {OFFICIAL_STORES.map((brand) => (
          <Link
            ...
            className="flex items-stretch overflow-hidden rounded-lg border border-tc-line
                       bg-white transition hover:shadow-[var(--shadow-tc-hover)]"
          >
            <span className="grid w-[84px] shrink-0 place-items-center bg-tc-ink px-2
                             text-center text-[13px] font-extrabold text-white">
              {brand.slice(0, 9)}
            </span>
            <span className="flex flex-col justify-center px-3 py-3">
              <span className="text-[15px] font-semibold text-tc-ink">{brand}</span>
              <span className="text-[13px] font-medium text-tc-link">Visit the store ›</span>
            </span>
          </Link>
        ))}
      </div>
...
export function FavouriteBrands() {
  return (
    <section className="rounded-lg bg-tc-bg py-8">
      <h2 className="mb-6 text-center text-3xl font-normal text-tc-ink">
      ...
      <div className="mx-auto grid max-w-[1300px] grid-cols-3 gap-3 px-4 sm:grid-cols-5 lg:grid-cols-7">
        {FAVOURITE_BRANDS.slice(0, 21).map((brand) => (
          <Link
            key={brand}
            href={`/search?q=${encodeURIComponent(brand)}`}
            className="grid h-[104px] place-items-center rounded-xl border border-tc-line
                       bg-white px-3 text-center text-[15px] font-extrabold text-tc-ink
                       shadow-sm transition hover:shadow-[var(--shadow-tc-hover)]"
          >
            {brand}
          </Link>
        ))}
...
export function SeoBlock() {
  return (
    <section className="rounded-lg bg-white p-5 text-[13px] leading-relaxed text-tc-ink/80">
      <h2 className="mb-2 text-[15px] font-bold text-tc-ink">
        Superior online shopping in Egypt
      </h2>
      <p>
        Offering a world-class customer experience, TopChoice is the top choice for online shopping in
        Egypt. From <Link href="/category/electronics" className="text-tc-link">electronics products</Link>{' '}
        to fashion, kids&apos; toys,{' '}
        <Link href="/category/baby" className="text-tc-link">baby products</Link>, homeware,
        furniture, sporting and outdoor products,{' '}
        <Link href="/category/beauty" className="text-tc-link">beauty products</Link>, books and
        stationery, groceries, and much, much more — the TopChoice Egypt store has millions of authentic
      ...
      <h3 className="mb-2 mt-4 text-[15px] font-bold text-tc-ink">
        Shop the best products &amp; top brands at TopChoice Egypt
      </h3>
      <p>
        If you&apos;re looking for a wide range of products from top brands, TopChoice Egypt has you
        covered. In our electronics department, you&apos;ll find{' '}
        <Link href="/category/mobiles" className="text-tc-link">mobile phones</Link>, wireless and
        in-ear <Link href="/category/audio" className="text-tc-link">headsets</Link>,{' '}
        <Link href="/category/laptops" className="text-tc-link">laptops</Link> and accessories,
        wearable devices, smartwatches,{' '}
        <Link href="/category/tv" className="text-tc-link">TVs</Link>, and a variety of{' '}
        <Link href="/category/gaming" className="text-tc-link">video games</Link>.
      </p>
    </section>
  );
}
